<template>
  <main>
    ...
    <div v-if="categories" class="container">
      <div v-for="(category) in categories" :key="category.id" class="column">
        <div class="box-cat">{{ category.category }}</div>
        <div v-for="clue in clues" :key="clue.id">
          <button
            v-if="category.category===clue.category"
            class="box-clue"
            id="show-modal"
...
export default {
  ...
  created() {
    ...
    getBoardClues(this.boardId).then(saved => {
      this.clues = saved;
    });
    getBoardCategories(this.boardId).then(saved => {
